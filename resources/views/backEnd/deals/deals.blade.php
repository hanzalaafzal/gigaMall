...
<div class="padding">
   <div class="box">
      <div class="box-header dker">
         <h3><i class="material-icons"></i> Create Deal</h3>
         <small>
         <a href="{{url('/admin')}}">Home</a> /
         <a href="">Deal Create</a>
         </small>
      </div>
      <div class="box-tool">
         <ul class="nav">
      ...
      <div class="box-body">
         <form method="POST" action="{{route('dealsStore')}}" accept-charset="UTF-8" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="form-group row">
               <label for="title_ar" class="col-sm-2 form-control-label">Deal Name	</label>
               <div class="col-sm-10">
                  <input placeholder="" class="form-control has-value" id="name" required=""name="deal_name" type="text">
               </div>
            </div>
            {{--  <div class="form-group row">
               <label for="title_ar" class="col-sm-2 form-control-label">Creation Date	</label>
               <div class="col-sm-10">
                  <input placeholder="" class="form-control has-value" id="name" required=""name="creation_date" type="text">
               </div>
            </div>  --}}
            <div class="form-group row">
               <label for="title_ar" class="col-sm-2 form-control-label">Expiry Date</label>
               <div class="col-sm-10">
                  <input placeholder="" class="form-control has-value" required=""name="expiry_date" type="date">
               </div>
            </div>
            <div class="form-group row">
...
{{--  @if(count($categories) > 0)  --}}
  <div class="padding">
     <div class="box">
        <div class="box-header dker">
           <h3><i class="material-icons"></i>Deals</h3>
           <small>
           <a href="{{url('/admin')}}">Home</a> /
           <a href="">Deals List</a>
           </small>
                   ...
                   <table class="table table-striped">
                      ...
                      <tbody>
                         @foreach($deals as $deal)
                          <tr>
                              {{--  <td>{{$deal->name}}</td>  --}}
                              <td class="text-center">{{$deal->deal_name}}</td>
                              <td class="text-center">{{$deal->creation_date}}</td>
                              <td class="text-center">{{$deal->expiry_date}}</td>
                              <td class="text-center">
                                @if($deal->deal_status == 'Active')
                                  <a href="{{route('dealDisable',$deal->id)}}" class="btn btn-danger btn-sm">Disable</a>
                                @else
                                  <a href="{{route('dealActive',$deal->id)}}" class="btn btn-success btn-sm">Activate</a>
                                @endif
                                <a href="{{route('dealEdit',$deal->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                <a href="{{route('dealDelete',$deal->id)}}" class="btn btn-danger btn-sm">Delete</a>
                              </td>
